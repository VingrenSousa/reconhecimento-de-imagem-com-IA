...
const Home = () => {
    ...
    async function novoObjetoTRaduzido(n: propsItem[]) {
        const receitas = []
        for (let v = 0; v < n.length; v++) {
            const ing = { ...n[v] }


            const response = await fetch(`https://api.mymemory.translated.net/get?q=${ing.name}&langpair=en-GBr|pt-BR`);
            const datajson = await response.json();
            const date = datajson.responseData.translatedText
